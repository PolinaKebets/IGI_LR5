...
# Review views — ТОЛЬКО ЭТИ 4 ФУНКЦИИ
def reviews(request):
    """Reviews list — только одобренные, плюс свои неодобренные для автора"""
    if request.user.is_authenticated:
        # Показываем одобренные + свои (даже неодобренные)
        reviews_list = Review.objects.filter(
            Q(is_approved=True) | Q(user=request.user)
        ).order_by('-date')
    else:
        # Анонимы видят только одобренные
        reviews_list = Review.objects.filter(is_approved=True).order_by('-date')
...
def delete_review(request, review_id):
    """Удаление своего отзыва (или админом любого)"""
    review = get_object_or_404(Review, id=review_id)
